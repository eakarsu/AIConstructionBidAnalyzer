/**
 * Custom Feature: insurance-recommend
 ...
 * Integration credentials: process.env.FEATURE_INSURANCE_RECOMMEND_KEY
...
const FEATURE_KEY = process.env.FEATURE_INSURANCE_RECOMMEND_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Liability insurance recommendation engine.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIConstructionBidAnalyzer - insurance-recommend',
    },
    body: JSON.stringify({
...
router.post('/insurance-recommend', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_INSURANCE_RECOMMEND_KEY');
    const ai = await callLLM({ feature: 'insurance-recommend', goal: 'Liability insurance recommendation engine', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['insurance-recommend', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[insurance-recommend] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'insurance-recommend',
      endpoint: '/api/ai/insurance-recommend',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[insurance-recommend] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/insurance-recommend/health', (req, res) => {
  res.json({
    feature: 'insurance-recommend',
    endpoint: '/api/ai/insurance-recommend',
