...
/**
 * Gap/CFS Feature Page: Real-time cost tracking with variance alerts
 * Slug: real-time-cost-tracking-with-variance-alerts
 * Backend: /api/cf-real-time-cost-tracking-with-variance-alerts
 */
export default function RealTimeCostTrackingWithVarianceAlertsPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-real-time-cost-tracking-with-variance-alerts', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Real-time cost tracking with variance alerts</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Ingest actuals (timesheets, material shipments); flag cost overruns.</p>
